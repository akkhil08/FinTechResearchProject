...
--
-- FIX: S_INIT_LT read port reduction
--
--   PROBLEM (previous version):
--     S_INIT_LT normal branch read 4 unique addresses in one cycle:
--       data_buf(i_cnt)           port A
--       data_buf(i_cnt+1)         port B
--       data_buf(M_act+i_cnt)     needs port C  -- does not exist
--       data_buf(M_act+i_cnt+1)   needs port D  -- does not exist
--     RAM64M only has 2 read ports → Vivado forced to make 2 copies
--
--   SOLUTION:
--     Split into S_INIT_LT (even pair, 2 reads) and
--     S_INIT_LT_ODD (odd pair, 2 reads) on alternating cycles.
--     Each state uses exactly 2 unique addresses → 1 RAM copy sufficient.
--
--     S_INIT_LT  reads: data_buf(i_cnt)       port A
--                       data_buf(M_act+i_cnt)  port B
--                       data_buf(i_cnt) reused for subtraction = same wire, not a new port
--
--     S_INIT_LT_ODD reads: data_buf(i_cnt+1)         port A
--                           data_buf(M_act+i_cnt+1)   port B
--                           skipped if i_cnt+1 >= M_act (odd M)
--
--   CYCLE COUNT IMPACT (M=12):
--     Previous dual-accumulator: 6 accumulation cycles + 1 divide = 7
--     This version:             12 accumulation cycles + 1 divide = 13
--     Cost: +6 init cycles (one time, negligible vs UPDATE pipeline)
--     Benefit: eliminates 2nd data_buf RAM copy from this state
--
...
entity holt_winters_q2_30 is
    ...
    -- FSM
    --
    -- S_INIT_LT_ODD is the new state that handles the odd-index pair read.
    -- It alternates with S_INIT_LT so each state sees only 2 unique
    -- data_buf addresses per cycle - exactly fitting RAM64M's 2 read ports.
    -- =========================================================================
    type state_t is (
        S_IDLE, S_COLLECT, S_VALIDATE, S_WAIT_FC,
        S_INIT_LT,
        S_INIT_LT_ODD,    -- NEW: odd-index pair, 2 reads only
        S_INIT_LT_DIV,
        S_SAVG,
        S_SAVG_DIV,
        S_SACC,
    ...
    -- =========================================================================
    -- Secondary accumulators (dual accumulator optimisation for SAVG / SACC)
    -- acc_L2 / acc_T2 accumulate odd-index elements in parallel with acc_L/T
    -- =========================================================================
    signal acc_L2 : fp_data := (others => '0');
    signal acc_T2 : fp_data := (others => '0');

    -- =========================================================================
    -- Season accumulator second channel
    -- S_acc2[j] accumulates odd-index seasonal deviations alongside S_acc[j]
    -- =========================================================================
    ...
    -- Replaces t_cnt mod M_act on the critical path (mod = divider = ~3-5ns)
    -- Wrap counter is a simple compare + increment = ~0.5ns
    -- =========================================================================
    ...
    -- use_dsp attribute forces DSP48E1 (2-3ns vs 6-8ns LUT multiply)
    -- =========================================================================
    ...
    -- Primary critical path function - called 4x per sample in S_UPDATE
    -- =========================================================================
    ...
    if rising_edge(clk) then
        ...
        if rst = '1' then
            ...
            case state is
                ...
                --
                -- data_buf(i_cnt) appears twice in this state:
                --   once added to acc_L
                --   once subtracted inside acc_T expression
                -- This is ONE read - the synthesiser reads the address once
                -- and fans the output wire to both consumers.
                -- Not a second port. Not a second copy.
                --
                -- When all pairs exhausted (i_cnt >= M_act):
                --   combine acc_L + acc_L2 and acc_T + acc_T2 into acc_L/T
                --   go to S_INIT_LT_DIV for the multiply
                -- =============================================================
                when S_INIT_LT =>
                    if i_cnt < M_act then
                        -- 2 unique reads this cycle: i_cnt and M_act+i_cnt
                        acc_L <= acc_L + data_buf(i_cnt);
                        acc_T <= acc_T + (data_buf(M_act + i_cnt)
                                        - data_buf(i_cnt));
                        -- go read the odd pair next cycle
                        state <= S_INIT_LT_ODD;
                    else
                        -- All pairs processed - combine both accumulators
                        -- No data_buf reads here: acc_L, acc_L2, acc_T, acc_T2
                        -- are all already in flip-flops
                        acc_L  <= acc_L + acc_L2;
                        acc_T  <= acc_T + acc_T2;
                        acc_L2 <= (others => '0');
                        acc_T2 <= (others => '0');
                ...
                --
                -- Port A: data_buf(i_cnt + 1)
                -- Port B: data_buf(M_act + i_cnt + 1)
                --
                -- Skipped (no reads) if i_cnt + 1 >= M_act.
                -- This handles odd M correctly - the last element was already
                -- captured by S_INIT_LT on the previous cycle.
                --
                -- Always advances i_cnt by 2 then returns to S_INIT_LT.
                -- The +2 step is what keeps even/odd reads separated.
                -- =============================================================
                when S_INIT_LT_ODD =>
                    if i_cnt + 1 < M_act then
                        -- 2 unique reads this cycle: i_cnt+1 and M_act+i_cnt+1
                        acc_L2 <= acc_L2 + data_buf(i_cnt + 1);
                        acc_T2 <= acc_T2 + (data_buf(M_act + i_cnt + 1)
                                          - data_buf(i_cnt + 1));
                    end if;
                    -- Always advance by 2 whether we read or not
                    -- (if M is odd the last i_cnt+1 was out of range - skip)
                    i_cnt <= i_cnt + 2;
                    state <= S_INIT_LT;
                ...
                --
                -- acc_L and acc_T now hold the fully combined sums.
                -- Multiply by inverse M to get L0 and T0.
                -- Separate from accumulation to keep timing paths short:
                --   combine (add) + multiply in same cycle = ~8ns
                --   multiply alone = ~3ns - well within budget
                -- =============================================================
                when S_INIT_LT_DIV =>
                    inv_M  := rcp(M_act);
                    ...
                    -- Reset accumulators for SAVG phase
                    S_acc  <= (others => (others => '0'));
                    S_acc2 <= (others => (others => '0'));
                    acc_L  <= (others => '0');
                    acc_L2 <= (others => '0');
                ...
                --
                -- 2 elements per cycle: acc_L (even) + acc_L2 (odd) in parallel
                -- M=12: 6 accumulation cycles + 1 divide = 7 cycles per season
                -- Reads: data_buf(idx) and data_buf(idx+1) = 2 unique addresses
                -- These are consecutive - easily served by 2 RAM ports
                -- =============================================================
                when S_SAVG =>
                ...
                -- =============================================================
                -- S_SAVG_DIV: divide cycle for season mean - no data_buf reads
                --
                -- cur_season_mean committed here as a registered signal.
                -- S_SACC reads it on the next clock edge - always stable.
                -- =============================================================
                when S_SAVG_DIV =>
                    inv_M           := rcp(M_act);
                ...
                --   mod synthesises as a divider  (~3-5ns critical path)
                --   wrap counter is compare+add   (~0.5ns)
                -- Single read from data_buf(t_cnt) - 1 unique address.
                -- S_arr(v_si) is a separate array - not data_buf.
                -- =============================================================
                when S_UPDATE =>
                    v_si  := si_counter;
                ...
                --
                -- si_counter incremented here with explicit wrap.
                -- fc_si_counter computed explicitly (not copied from
                -- si_counter) to avoid reading the pre-update value.
                -- =============================================================
                when S_UPDATE_WAIT =>
                    ...
                    -- Increment si_counter with explicit wrap
                    ...
                    if upd_t = N_act - 1 then
                        -- fc_si_counter must be computed explicitly here.
                        -- Assigning fc_si_counter <= si_counter would capture
                        -- the OLD si_counter value (before the wrap above
                        -- takes effect) because all <= assignments in a clocked
                        -- process read the current register values, not the
                        -- newly scheduled values. The explicit if/else below
                        -- mirrors the wrap logic and produces the correct
                        -- post-wrap value.
                        if si_counter = M_act - 1 then
                            fc_si_counter <= 0;
                        else
                ...
                -- =============================================================
                -- S_FORECAST: uses fc_si_counter - no mod, no divider
                -- =============================================================
                when S_FORECAST =>

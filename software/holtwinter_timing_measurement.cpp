...
int main() {
    ...
    // ── GA optimisation (not timed — this runs offline) ──────────────────────
    cout << "\nOptimizing additive parameters with GA...\n";
    GAParams ga_add = run_ga_optimize(train_vals, season_len, true);
    ...
    GAParams ga_mul = run_ga_optimize(train_vals, season_len, false);
    ...
    // ── CPU TIMING — averaged over 1000 runs ─────────────────────────────────
    // We time only the Holt-Winters forecasting kernel (parameters already known).
    // This matches what the FPGA does: 60 samples in → 12 forecasts out.
    // The GA runs offline once; only the inference step runs in real-time.

    const int TIMING_RUNS = 1000;
    long long total_ns = 0;

    HWResult add_orig_timed, mul_orig_timed;

    for (int run = 0; run < TIMING_RUNS; ++run) {
        auto t_start = std::chrono::high_resolution_clock::now();

        // Additive forecast with GA-optimised parameters
        HWResult tmp_add = holt_winters_additive(
            train_vals, season_len, horizon,
            add_alpha, add_beta, add_gamma, false
        );

        // Multiplicative forecast with GA-optimised parameters
        HWResult tmp_mul = holt_winters_multiplicative(
            train_vals, season_len, horizon,
            mul_alpha, mul_beta, mul_gamma, false
        );
        ...
        // Keep last run's results for downstream use
        if (run == TIMING_RUNS - 1) {
            add_orig_timed = tmp_add;
            mul_orig_timed = tmp_mul;
        }
    }

    double avg_ns = (double)total_ns / TIMING_RUNS;
    double avg_us = avg_ns / 1000.0;

    cout << "\n========== CPU TIMING RESULTS (" << TIMING_RUNS << " runs) ==========\n";
    cout << fixed << setprecision(3);
    cout << "Average Holt-Winters forecast time : " << avg_ns << " ns\n";
    cout << "Average Holt-Winters forecast time : " << avg_us << " us\n";
    cout << "FPGA end-to-end latency            : 6500.000 ns  (6.500 us)\n";
    if (avg_ns > 6500.0)
        cout << "CPU is SLOWER than FPGA by       : "
             << (avg_ns - 6500.0) << " ns\n";
    else
        cout << "CPU is FASTER than FPGA by       : "
             << (6500.0 - avg_ns) << " ns  "
             << "(see presentation notes — FPGA still wins on determinism & parallelism)\n";
    cout << "=====================================================\n\n";
    // ─────────────────────────────────────────────────────────────────────────

    HWResult add_orig = add_orig_timed;
    HWResult mul_orig = mul_orig_timed;
    ...
    double sum_abs_err_orig = 0.0;
    double sum_abs_err_log  = 0.0;
    int    count_orig       = 0;
    int    count_log        = 0;
    ...
    for (int i = 0; i < horizon; ++i) {
        ...
        if (!std::isnan(abso)) { sum_abs_err_orig += abso; count_orig++; }
        if (!std::isnan(absl)) { sum_abs_err_log  += absl; count_log++;  }
    ...
    double mae_orig = (count_orig > 0) ? (sum_abs_err_orig / count_orig)
                                       : numeric_limits<double>::quiet_NaN();
    double mae_log  = (count_log  > 0) ? (sum_abs_err_log  / count_log)
                                       : numeric_limits<double>::quiet_NaN();

    cout << "\nMean Absolute Error (Original-scale forecast vs actual): " << mae_orig << "\n";
    cout << "Mean Absolute Error (Log-back-transformed forecast vs actual): " << mae_log << "\n";

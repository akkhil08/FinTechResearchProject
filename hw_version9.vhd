-- =============================================================================
-- holt_winters_q2_30_opt_v8.vhd
--
-- v8 THROUGHPUT FIX (builds on v7):
--   FIX 3: S_UPDATE_PRE folded into S_UPDATE_FINALIZE for iterations 2..N.
--           Only the very first sample still passes through S_UPDATE_PRE
--           (entry point from S_SACC). All subsequent samples jump directly
--           S_UPDATE_FINALIZE -> S_UPDATE, saving 1 cycle per sample.
--           Throughput: 5 cycles/sample -> 4 cycles/sample (-20%).
--           No new arithmetic on FINALIZE critical path: the two added array
--           reads (distributed RAM, 1 LUT deep) leave ample timing margin.
--           Safe because M_act >= 2 is enforced in S_VALIDATE, so
--           next_si != upd_si always holds (no RAW hazard on S_arr).
-- =============================================================================
...
architecture rtl of holt_winters_q2_30 is
    ...
    -- FIX 1: upd_data_wait - pre-fetched in S_UPDATE_MULT
    ...
    if rising_edge(clk) then
        ...
        if rst = '1' then
            ...
            case state is
                ...
                -- =============================================================
                -- PRE-FETCH: Entry point for the FIRST sample only (t=0).
                -- All subsequent samples are pre-fetched inside S_UPDATE_FINALIZE
                -- (FIX 3), so this state executes exactly once per run.
                -- =============================================================
                when S_UPDATE_PRE =>
                ...
                when S_UPDATE_MULT =>
                    ...
                    -- FIX 1: Pre-fetch data_buf(upd_t) here
                ...
                when S_UPDATE_WAIT =>
                    ...
                    -- FIX 2: Capture L_reg before overwriting
                    prev_L <= L_reg;
                    L_reg  <= upd_newL;
                    ...
                    -- FIX 1: Use pre-fetched upd_data_wait
                ...
                when S_UPDATE_FINALIZE =>
                    ...
                    if upd_t = N_act - 1 then
                        ...
                        -- =======================================================
                        -- FIX 3: Pre-fetch next sample's operands here instead of
                        -- transitioning to S_UPDATE_PRE. This saves 1 cycle for
                        -- every sample after the first (N-1 savings total).
                        --
                        -- t_cnt was set to upd_t+1 in S_UPDATE, so it already
                        -- holds the next sample index.
                        --
                        -- next_si != upd_si is guaranteed (M_act >= 2), so reading
                        -- S_arr(next_si) while writing S_arr(upd_si) is safe.
                        -- =======================================================
                        upd_S_pre    <= S_arr(next_si);
                        upd_data_pre <= data_buf(t_cnt);
                        upd_si       <= next_si;
                        upd_t        <= t_cnt;
                        state        <= S_UPDATE;   -- skip S_UPDATE_PRE
                    end if;

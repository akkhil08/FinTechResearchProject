...
--
-- PORT CHANGE: m_in and horizon_in removed from entity ports.
--   M_act    is now assigned from generic MAX_M     in S_IDLE.
--   HORIZON_act is now assigned from generic MAX_HORIZON in S_WAIT_FC.
--   Callers control M and HORIZON by setting the generics at instantiation.
-- =============================================================================
    ...
    if rising_edge(clk) then
        ...
        if rst = '1' then
            ...
            case state is
                ...
                -- =============================================================
                -- M_act now comes from the MAX_M generic (no m_in port).
                -- =============================================================
                when S_IDLE =>
                    if start = '1' then
                        M_act      <= MAX_M;
                ...
                -- =============================================================
                -- HORIZON_act now comes from the MAX_HORIZON generic (no horizon_in port).
                -- =============================================================
                when S_WAIT_FC =>

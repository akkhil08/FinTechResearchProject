    ...
    if rising_edge(clk) then
        ...
        if rst = '1' then
            ...
            case state is
                ...
                -- =============================================================
                -- THE FIX: Pre-fetch data to break the array routing delay
                -- =============================================================
                when S_UPDATE_PRE =>
                ...
                when S_UPDATE_WAIT =>
                    ...
                    mul_beta_tmp  <= fp_mul_param(beta,  upd_newL - L_reg);
                    mul_1mb_tmp   <= fp_mul_param(v_1mb, T_reg);
                    mul_gamma_tmp <= fp_mul_param(gamma, data_buf(upd_t) - upd_newL);
